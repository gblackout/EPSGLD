...
def process_data_yy(input, output, ind_bak, jump_id=-1):
    """ jump_id: id that <= this value will be omitted"""
    global prefix

    if input.lower().endswith("bz2"):
        opener = bz2.BZ2File
    else:
        opener = open
...
def WikiDocument(out, id, title, text):
    """ text is the plain text in one article without header and footer"""
...
def wiki_doc(out, text):
    """ text is the plain text in one article without header and footer"""

    text = clean(text)
    # tokenized into ['', '']
    text = set_util.remove_comma_outter(text)
    out.append(text)


class out_splitter_yy:
    def     __init__(self, doc_per_pkl, path_name, index):
        self.file_index = index
        self.doc_per_pkl = doc_per_pkl
        self.path_name = path_name
        self.cnt = 0
        self.doc_set = []

    def append(self, doc):
        if self.cnt == self.doc_per_pkl:
            self.dump()
            self.cnt = 1
            self.file_index += 1
        else:
    ...
    def dump(self):
        if len(self.doc_set) != self.doc_per_pkl:
            pickle.dump(self.doc_set, open(self.path_name + 'save_' + str(self.file_index) + 'incomplete', 'w'))
        else:
            pickle.dump(self.doc_set, open(self.path_name + 'save_' + str(self.file_index), 'w'))
        self.doc_set = []



class OutputSplitter:
    def __init__(self, compress, max_file_size, path_name, index):
        self.dir_index = 0
        self.file_index = index
    ...
    def reserve(self, size):
        """ judge if current file is full, open new file if true"""
...
def main():
    ...
    # process_data(sys.stdin, compress, file_size, output_dir)
    # output_splitter.close()
    """ args: file_name, index, jump_file_id"""
    if len(sys.argv) != 4:
        print 'try again'
    else:
        ind_bak = [0]
        try:
            output_splitter  = out_splitter_yy(doc_per_pkl=10000, path_name='./saved/'+sys.argv[1][28:30]+'_', index=int(sys.argv[2]))
            process_data_yy(input=sys.argv[1], output=output_splitter, ind_bak=ind_bak, jump_id=int(sys.argv[3]))
            output_splitter.dump()
        except Exception as e:
            print  e
        except IOError as e:
            print e
            print 'IOError'
            print 'current doc:', ind_bak[0]
        finally:
            print 'current doc:', ind_bak[0]
